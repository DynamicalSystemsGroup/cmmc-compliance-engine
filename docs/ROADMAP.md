...
- [x] Audit-package report landed: `ce report` (and `ce demo --report`) renders the signed manifest into a self-contained `report.html`, plus a paged `report.pdf` when the `weasyprint` binary is present. Code: `documents/report.py`.
