...
- [x] Attested-reference oracle wired into the run path with teeth (`oracles/attested_reference.py`, `pipeline/runner.py` `_run_attested_reference_pass`): Track B (policy/human) controls are now gated MET on the oracle passing (reference registered, resolves to a real file, fresh, role-signed), emitting `passed`/`needsAction`/`failed` instead of a bare `cantTell`. `pipeline/evidence/doc_evidence.py` resolves each control's `ce:Reference` to the real document on disk, SHA-256 hashes it, captures the git commit that last changed it, and signs an upload receipt (local Ed25519 dev signer), binding a `ce:DocumentEvidence` node into the evidence graph. This adds the BOM `attested-evidenced` backing; over `ce demo --full` the distribution is 49 machine / 43 attested-evidenced / 18 human-only. Evidence remains fixture/mock (runs stay NON-EVIDENTIARY).
